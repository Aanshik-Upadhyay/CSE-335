 ...
 * @brief FishBubbles class for our aquarium
 */
 ...
 */
FishBubbles::FishBubbles(Aquarium *aquarium) : Item(aquarium)
{
 mFishImage = make_unique<wxImage>(FishBubblesImageName, wxBITMAP_TYPE_ANY);
 ...
 * @return true if hit.
 */
bool FishBubbles::HitTest(const int x, const int y)
{
 const double wid = mFishBitmap->GetWidth();
 const double hit = mFishBitmap->GetHeight();
